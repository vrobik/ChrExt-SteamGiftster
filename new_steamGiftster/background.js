...
function UpdateActivity() {
	...
	for (var i = 1; i < 19; ++i) {
		maxEntered = Math.max(maxEntered, EntriesPerHour[i]);
		maxContests = Math.max(maxContests, ContestsPerHour[i]);
	}
	if (maxContests == 0) {
		return
	}
...
function MessageHandler(request, sender, sendResponse) {
	...
	if( request.type == "won_game" ){
		iconContext.clearRect(0, 0, 19, 19);
		iconContext.drawImage(iconImageWon, 0, 0, 19, 19);
		var iconWon = iconContext.getImageData(0, 0, 19, 19);
		chrome.browserAction.setIcon({imageData: iconWon});
	}else {
				...
				if (request.type == "scrape_results") {
					...
					if (request.data.results.last != request.data.results.total) {
						nextThingToDo = function () {
							++page;
							chrome.tabs.update(sender.tab.id, {url: "http://www.steamgifts.com/giveaways/search?page=" + page});
						...
						for (var i = 0; i < contests.length; ++i) {
							var pIndex = priorities.indexOf(contests[i].title); 	// id-ul jocului la momentul dat in lista de prioritati
							var numEntries = 0; 									// count de jocuri de acelas fel
							for (var j = 0; j < contests.length; ++j) {
								if (contests[i].title == contests[j].title && contests[j].entered) {
									numEntries++
								}
							}
							if (pIndex === -1) {									// daca nu avem jocul in lista
								priorities.push(contests[i].title);					// il adaugam cu prioritatea egala cu numarul de GV
				...
				if (thing.action == "scrape_posts") {
					...
					if (thing.action == "scrape_forums") {
						...
						if (thing.action == "scrape_open") {
							doingSomething = true;
							chrome.tabs.create({windowId: wId, url: "http://www.steamgifts.com/giveaways/search?page=1", index: 9999, active: false}, function (newTab) {
